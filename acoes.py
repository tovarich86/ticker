...
# Tenta importar o engine local. Se falhar, avisa o usuário.
try:
    import b3_engine
except ImportError:
    st.error("ERRO CRÍTICO: O arquivo 'b3_engine.py' não foi encontrado na mesma pasta.")
    st.stop()

# --- 1. CONFIGURAÇÃO GLOBAL DA PÁGINA ---
# Deve ser a primeira chamada Streamlit do script
st.set_page_config(page_title="Portal Market Data (B3/Yahoo)", layout="wide")

# ==============================================================================
# MÓDULO 1: LÓGICA DE TICKERS (acoes.py)
# ==============================================================================
...
def render_page_tickers():
    st.title('Consulta Consolidada de Dados (Ações/FIIs)')
    st.markdown("""
    **Fontes:**
    ...
    * **Yahoo Finance:** Dados internacionais e Fechamento Ajustado.
    ...
    with col1: tickers_input = st.text_input("Tickers (ex: PETR4, AAPL, ITUB4):", placeholder="Separe por vírgula")
    ...
    # Exibição dos Resultados (Persistente via Session State)
...
def processar_data_di(data, session):
    """Busca e processa dados de DI."""
    ...
    try:
        ...
        df = pd.read_csv(BytesIO(csv_bytes), sep=';', encoding='latin1', decimal=',', engine='python')

        if df.empty: return None, "Vazio"
        df.columns = [col.strip() for col in df.columns]
        df.insert(0, "DATA REFERÊNCIA", data_formatada)
        mapa = {'Descrição da Taxa': 'DESCRICAO', 'Dias Úteis': 'DIAS_UTEIS', 'Dias Corridos': 'DIAS_CORRIDOS', 'Preço/Taxa': 'TAXA'}
...
def render_page_di():
    ...
    st.sidebar.subheader("Configurações DI")
    ...
    if modo_consulta == 'Data Única':
        ...
        uploaded_file = st.sidebar.file_uploader("Carregue arquivo (CSV/XLSX)", type=['csv', 'xlsx'])
        if uploaded_file:
    ...
    if st.sidebar.button("Baixar Taxas DI", type="primary"):
        if not datas_a_processar:
            ...
            for i, data in enumerate(datas_a_processar):
                df, status = processar_data_di(data, session)
                if df is not None: lista_dfs.append(df)
                else: st.error(f"{data}: {status}")
            ...
            if lista_dfs:
                ...
                st.download_button("📥 Baixar em Excel", data=output.getvalue(), file_name=f"Taxas_DI_{datetime.now().strftime('%Y%m%d')}.xlsx")
...
with st.sidebar:
    st.header("📌 Navegação")
    # Usa selectbox ou radio para alternar entre as ferramentas
    pagina_escolhida = st.radio("Escolha a Ferramenta:", ["Busca de Tickers", "Taxas DI1 B3"])
    st.markdown("---")

if pagina_escolhida == "Busca de Tickers":
    render_page_tickers()
else:
    render_page_di()
...
# RODAPÉ COMUM
st.markdown("---")
st.caption("Ferramenta Unificada | Fonte dos dados: B3 e Yahoo Finance.")
